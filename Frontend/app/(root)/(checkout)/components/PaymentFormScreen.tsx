...
const PaymentFormScreen = ({ type, onSave, onCancel, useCheckoutContext = true, initialValues }: PaymentFormProps) => {
  ...
  const [card, setCard] = useState(() => initialValues && type === 'card' ? {
    type: initialValues.type || 'VISA',
  ...
  const [mobileMoney, setMobileMoney] = useState(() => initialValues && type === 'mobile' ? {
    network: initialValues.network || 'MTN',
      ...
      <View className="flex-1">
        <ScrollView className="px-4 pt-4 pb-24">
          ...
          <View style={{ marginTop: 32 }}>
            <PrimaryButton BtnText='Save' disabled={type === 'card' ? !isCardValid : !isMobileValid} onPress={handleSave}/>
