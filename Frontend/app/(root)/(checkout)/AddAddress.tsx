...
import { fetchAddresses, updateAddress, Address, addAddress } from '@/app/api/addressApi';
import { BASE_URL } from '@/constants/env';
import { countries } from '@/constants/countries';

// Helper: create a mapping from country name to id and id to name/code
...
const AddAddressScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (editMode && params.addressId && user?.id && token) {
      const loadAddress = async () => {
        try {
          ...
          console.log('Address to prefill:', address, 'params.addressId:', params.addressId);
          if (address) {
            ...
            console.log('Prefilling edit address form with:', address);
          }
        } catch (error) {
  ...
  }, [editMode, params.addressId, user, token]);
  ...
  const validateForm = () => {
    ...
    // Simple phone validation: must be digits, 8-15 chars
    if (!/^[0-9+]{8,15}$/.test(formData.phone)) {
      setErrorMsg('Phone number must be 8-15 digits.');
      return false;
    }
    // Simple zip code validation: must be digits, 3-10 chars
  ...
  const handleSave = async () => {
    ...
    try {
      console.log('Saving address with country:', formData.country, 'countryId:', countryNameToId[formData.country]);
      const addressPayload: Partial<Address> = {
        ...formData,
        countryId: countryNameToId[formData.country] || 66, // fallback to Ghana
        id: editMode && typeof params.addressId === 'string' ? params.addressId : undefined,
      };
      console.log('Address payload for save:', addressPayload);
      if (editMode && typeof params.addressId === 'string') {
        await updateAddress(addressPayload, token);
      } else {
        const newAddress = await addAddress(addressPayload, token);
        // Link address to user
        console.log('Linking address to user:', {
          userId: user?.id,
          addressId: newAddress.id,
          isDefault: true
        });
        const linkRes = await fetch(`${BASE_URL.replace(':8080', ':8085')}/api/addresses/link`, {
          method: 'POST',
          headers: {
        ...
        const linkResJson = await linkRes.json().catch(() => ({}));
        console.log('Link address response:', linkRes.status, linkResJson);
      }
      // Always return to SavedAddressScreen with router.back()
      router.back();
    } catch (e: any) {
      setErrorMsg(e.message || 'Failed to save address. Please try again.');
    }
  };
