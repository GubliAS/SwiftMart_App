...
const AddPaymentScreen = () => {
  ...
  const handleSave = async () => {
    ...
    try {
    if (type === 'card') {
        if (!isCardValid) throw new Error('Invalid card');
        let expiryDate = '';
      ...
      await addPaymentMethod(cardData, token);
        ...
        if (!validatePhoneNumberForNetwork(mobileMoney.phone, mobileMoney.network)) throw new Error('Invalid phone');
      const mobileData = {
        userId: user.id,
          paymentTypeId: 4,
        provider: mobileMoney.network,
        accountNumber: mobileMoney.phone,
        isDefault: false,
      };
 ...
 <SafeAreaView className="flex-1 bg-white gap-8 py-4">
        ...
        <ScrollView className="px-4 pt-4 pb-24">
          ...
          <View style={{ marginTop: 32 }}>
            <PrimaryButton
              BtnText={type === 'card' ? 'Add Card' : 'Confirm'}
              onPress={handleSave}
              loading={loading}
