      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-yellow-50 p-4 rounded-2xl mb-6">
          ...
          <View className="flex-row gap-2">
            <Pressable
              className="flex-1 bg-yellow-400 py-2 rounded-lg items-center"
              onPress={() => router.push("/(auth)/ProcessingStatusScreen")}
            >
        ...
        {/* Modified this section to only render one item (Ereko.png) */}
        {[1].map((_, index) => ( // Changed [1, 2] to [1]
          <View
            key={index}
            className="flex-row items-center bg-white p-4 mb-4 rounded-xl shadow-sm"
          >
            <Image
              source={require("@/assets/images/Ereko.png")} // Directly specify Ereko.png
